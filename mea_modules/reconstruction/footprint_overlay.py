...
(Adam: the functional plotting belongs in ``mea_modules``, imported by a thin
...
def overlay_reconstruction(ax, *, polylines, text_color: str, branch_velocity=None,
    ...
    node_kw = dict(polylines=polylines, channels_per_branch=channels_per_branch,
                   ...
                   soma_color=soma_color, branchpoint_color=branchpoint_color,
    ...
    if graded and channels_per_branch is not None and len(channels_per_branch):
...
def render_footprint_reconstruction_panel(
    ...
    fig, ax, *_rest = plots._render_footprint_core(
        ...
        scale_circle_value=True,  # the grant panel shows the explicit max µV (Adam, 2026-08-25)
    )
    text_color = _rest[-1]
