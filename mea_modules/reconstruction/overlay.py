...
**What this replaces.** The old build's
...
with one color per neuron as the main difference from the individual
reconstructions: black background with white
...
every encoding on the figure itself (the module's standing legend contract,
2026-08-11 — no insider jargon, acronyms defined, n shown vs n excluded and
why).

**Poster note (2026-08-12).** This figure goes on a printed
...
# Initiation-site marker: a filled diamond ("D"), chosen over a star marker
# after the first render made stars read poorly as soma points. The WIDE
# variant ("D", not the thin "d") — at poster scale the
...
def _compose_caption(*, well_label, n_drawn, n_no_branch, n_upstream,
    ...
    reasons — the module's standing legend contract (2026-08-11), the same contract the
...
def plot_all_reconstructions(records, out_path, *, locations=None,
    ...
    states every exclusion and why, per the module's standing legend contract. Units whose record
    ...
    one-color-per-neuron, scale bar and clean µm axes all stay. The
    diagnostic default is unchanged (2026-08-12) — the verbose review figure
    is still needed for review).
    ...
    # style="presentation" (2026-08-12): the deck-ready cut of this same
    ...
    # and one-color-per-neuron stay untouched, as do
    # the clean µm axes and the scale bar. The diagnostic default is
    # unchanged — the verbose review figure is still needed for review.
    # `background` stays its own exposed knob (default black), so
    # presentation keeps this look unless a caller passes background="white".
    ...
    try:
        ...
        for record, color in zip(drawn, colors):
            ...
            if record.get("init_xy") is not None:
                # "D" (wide diamond), the chosen marker — see
        ...
        # identity belongs on the slide (2026-08-12) — and instead
        ...
        if presentation:
            ...
            # (those go on the slide, 2026-08-12).
        ...
        # Caption at the bottom edge — the figure explains itself (the
        # module's standing legend contract). Anchored just BELOW the canvas
        # (negative y, va="top"):
